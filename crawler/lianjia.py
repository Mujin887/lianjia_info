...
def trade_spider(area_arg):
    ...
    for location in link.find_all('a'):
        ...
        for inner_location in area_link.find_all('a')[area_offset:]:
            location_url = "http://bj.lianjia.com"+inner_location.get('href')
            print(location.string,location_url)
            # location_soup = get_url_soup(location_url)
            inner_area= location_url.split("/")[-2]
            print(inner_area)
            # set_area_sum_data(area + ":" +inner_area, location_soup)
...
def set_area_sum_data(area, area_soup):
    ...
    if "暂无数据" in str(today_check):
        today_check = str(0)
    if "暂无数据" in str(on_sale):
        on_sale = str(0)
    if "暂无数" in str(ninety_days_sold):
        ninety_days_sold = str(0)

    area_key = module_prefix + area
    area_value = date_score + "," + today_sold + "," + today_check + "," + on_sale + "," + ninety_days_sold
    print("key: ", area_key, "\nscore: ", date_score + "\nvalue: " + area_value)
...
date_score = (datetime.now() + timedelta(days = -1)).strftime("%Y%m%d") # 由于部署服务器在美国，时区的关系不需要减去1天
# date_score = datetime.now().strftime("%Y%m%d") # TODO:由于部署服务器在美国，时差的关系不需要减去1天，上线时调整过来
